...
export default function PostRequest() {
  ...
  const exampleChips = ["I need a barber who can cut my hair tonight in Orlando", "Looking for a logo designer for my startup", "Need a plumber to fix a leaky faucet ASAP", "Seeking a tutor for calculus online"];
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-accent/30 to-background py-12 px-4">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="prompt" placeholder="e.g., I need a barber who can cut my hair tonight in Orlando..." value={promptText} onChange={e => setPromptText(e.target.value)} className="min-h-32 text-base" />
              ...
              <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    {localized ? <MapPin className="h-5 w-5 text-primary" /> : <Globe className="h-5 w-5 text-primary" />}
                    <div>
                      <Label htmlFor="localized" className="font-semibold">
                        {localized ? "In-Person Service" : "Remote Service"}
                      </Label>
                      <p className="text-xs text-muted-foreground">
                        {localized ? "For services that require physical presence" : "For digital, design, coaching, or remote work"}
                      </p>
                    </div>
                  </div>
                  <Switch id="localized" checked={localized} onCheckedChange={setLocalized} />
                </div>

                {localized && <div className="space-y-2 pt-2 border-t">
                    <Label htmlFor="location">Location</Label>
                    <div className="flex gap-2">
                      <Input id="location" placeholder="Enter city, ZIP, or address" className="flex-1" />
              ...
              <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <Switch id="sms" checked={smsAddon} onCheckedChange={setSmsAddon} />
                </div>

                {smsAddon && <div className="space-y-2 pt-2 border-t">

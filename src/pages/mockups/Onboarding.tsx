...
export default function Onboarding() {
  ...
  return <div className="min-h-screen bg-background flex items-center justify-center p-6">
      ...
      {!isLastSlide && <Button variant="ghost" onClick={handleSkip} className="fixed top-4 right-4 z-50 text-muted-foreground hover:text-foreground">
          Skip
        </Button>}

      <div className="w-full max-w-md">
        <Carousel setApi={handleCarouselChange} opts={{
        loop: false,
        align: "center"
      }} className="w-full">
          <CarouselContent>
        ...
        <div className="flex justify-center gap-2 mt-8">
          {slides.map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={`h-2 rounded-full transition-all duration-300 ${current === index ? "w-8 bg-primary" : "w-2 bg-muted-foreground/30"}`} aria-label={`Go to slide ${index + 1}`} />)}
        </div>

        {/* CTA Buttons - Only on last slide */}
        {isLastSlide ? <div className="space-y-3 pt-12 animate-fade-in">
            <Button size="lg" onClick={handleGetStarted} className="w-full h-14 text-base font-semibold rounded-2xl shadow-lg">
              Sign Up or Log In
            </Button>

            <Button variant="outline" size="lg" onClick={handleContinueAsGuest} className="w-full h-14 text-base font-semibold rounded-2xl border-2">
              Continue as Guest
            </Button>
          </div> : <div className="pt-12">
            <Button size="lg" onClick={handleNext} className="w-full h-14 text-base font-semibold rounded-2xl shadow-lg">
              Next
            </Button>
          </div>}

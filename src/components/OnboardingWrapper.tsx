...
export const OnboardingWrapper = ({ children }: OnboardingWrapperProps) => {
  ...
  useEffect(() => {
    ...
    // Skip onboarding check for mockup routes and auth route
    if (location.pathname.startsWith('/mockup') || location.pathname === '/auth') {
      setChecked(true);
      return;
    }

    // If user is not authenticated and hasn't seen onboarding, redirect to onboarding
    if (!user && !hasSeenOnboarding() && location.pathname !== '/mockup/onboarding') {
      // Store the intended destination
      const intendedPath = location.pathname !== '/' ? location.pathname : null;
      if (intendedPath) {
        sessionStorage.setItem('intended_path', intendedPath);
      }
      navigate('/mockup/onboarding', { replace: true });
    }
